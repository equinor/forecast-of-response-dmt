...
function ModeData({rao, motInd}) {
  ...
  for (var i = 0; i<rao.directions.length; i++){
    ...
    var stInd = motInd*rao.directions.length*rao.frequencies.length + i*rao.frequencies.length;          
    var enInd = stInd + rao.frequencies.length;
    for (var j = stInd; j<enInd; j++){
      trace.y.push(Math.sqrt(Math.pow(rao.realvalues[j],2) + Math.pow(rao.imagValues[j],2)) )
    }
